...
// ─────────────────────────────────────────────────────────────────────────────
// ja — loaded on demand by the loader in index.html.
LOCALES['ja'] = {
  ...
  folder_add_as_space_title: 'スペースとして追加?',
    ...
    settings_label_push: 'プッシュ通知（アプリを閉じていても届きます）',
    settings_desc_push: 'ブラウザを閉じていても通知します — ターンの完了時や、エージェントが承認を待っているときに。',
    push_test_btn: 'テスト通知を送信',
    push_ios_hint: 'iPhone と iPad では、Hermes をホーム画面に追加した後でのみプッシュが機能します。Safari のタブでは受信できません。',
    push_active: 'この端末で有効です。',
    push_unsupported: 'このブラウザはプッシュに対応していません。',
    push_needs_https: 'プッシュには HTTPS（または localhost）が必要です。',
    push_permission_denied: '通知の許可が拒否されました。',
    push_no_service_worker: 'Service worker を利用できません。',
    push_server_unavailable: 'サーバーがプッシュキーを返せませんでした。',
    push_test_sent: 'テスト通知を送信しました。',
    push_test_failed: 'テスト通知に失敗しました。',
